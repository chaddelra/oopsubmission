    ...
    private void calculatePayroll() {
        try {
            ...
            if (this.grossSalary <= 0) {
                // If no attendance records are found or calculation failed, use the standard working hours
                // Assuming 8 hours per day, 22 working days per month as the default
                double standardMonthlyHours = 8 * 22; // 176 hours per month
                this.grossSalary = standardMonthlyHours * hourlyRate;
                System.out.println("No valid attendance records found. Using standard monthly hours (" + 
                    standardMonthlyHours + " hours) × hourly rate (" + hourlyRate + "): " + grossSalary);
            }
            ...
            this.netMonthlyPay = grossSalary + totalAllowances - totalDeductions;
