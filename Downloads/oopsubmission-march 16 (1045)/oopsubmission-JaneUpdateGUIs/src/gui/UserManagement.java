...
public class UserManagement extends javax.swing.JFrame {
    ...
    private void initializeTable() {
        tableModel = (DefaultTableModel) UserMgmtTbl.getModel();
    ...
    if (!foundMatch) {
        ...
        private String getNextEmployeeID() {
            ...
            // Return the next ID (current highest + 1)
        ...
        private String generateEmail(String firstName, String lastName) {
            // Format: lastName_firstName@mph.com (all lowercase)
            return (lastName + "_" + firstName + "@mph.com").toLowerCase();
        }
    ...
    private void editUserBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editUserBtnActionPerformed
        var selectedRow = UserMgmtTbl.getSelectedRow(); // Get selected row
    if (selectedRow >= 0) {
   ...
   //This handles the "create new user" button action.
    ...
    if (result == JOptionPane.OK_OPTION) {
        ...
        if (isEmployeeIDDuplicate(id) && !id.equals(suggestedID)) {
            int choice = JOptionPane.showConfirmDialog(this,
                "Employee ID " + id + " already exists. Use suggested ID " + suggestedID + " instead?",
                "Duplicate ID", JOptionPane.YES_NO_OPTION);
